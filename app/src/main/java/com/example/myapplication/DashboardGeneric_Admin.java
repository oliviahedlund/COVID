...
public class DashboardGeneric_Admin extends Fragment {
    ...
    private void getCenters(String county,Spinner Center_dropdown){
        ...
        String[] Centers = new String[]{"Choose citron"};// sätt till enbart {}
        int i=0,j=0;
        /*
        while(list_of_centerIDs != NULL){ //sorts the centers depending on county
            if(list_of_centerIDs[i].getCounty().equals(county)){
                Centers[j] = list_of_centerIDs[i].getCounty();
                j++;
            }
            i++;
        }
